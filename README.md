...
> 🚀 **云服务体验即将上线** —— 上线后可直接在线问答、无需本地安装;当前请用 [快速开始](#快速开始) 本地体验,部署方法见 [部署到云端](#部署到云端)。
...
> 🚀 **即将上线** —— 在线演示服务正在准备中,上线后无需安装即可体验;当前请先用 [快速开始](#快速开始) 本地运行。

上线后的云演示将提供:
- **内置演示知识库**:10 份虚构企业文档(晨光科技)已预置进镜像,开箱可检索;
- **注册即用**:创建账号即可提问,无需自配任何模型 / 密钥;
- 示例问题:「差旅报销需要准备哪些材料?」「密码口令有什么要求?」「年休假怎么算?」——回答自动标注 `[n]` 文件级引用,来源面板按文件分组。

> ⚠️ **演示说明**:语料为脚本生成的虚构内容,仅供学习演示;未挂持久化卷时,实例重建后用户与对话记录可能清空;访客注册会消耗部署者的 LLM 配额,请留意成本。部署与安全细节见 [部署到云端](#部署到云端)。

## 部署到云端

本项目可直接部署到任意支持 Docker 的平台。镜像在构建期已**预生成演示语料 + 本地 BGE 索引**,云实例一启动即可检索,无需部署后再上传文档或配嵌入 key。流程:

1. **推送并关联仓库**:把代码推到 GitHub,在平台控制台「新建项目 → 从 Git 导入仓库」并识别 Dockerfile 构建。
2. **服务端口**:网络设置把端口设为 `8001`(镜像内已 `EXPOSE 8001`)。
3. **环境变量**(必须与镜像内建索引的嵌入配置一致,否则前端提示「需要重建索引」):

| 变量 | 值 | 说明 |
|---|---|---|
| `EMBEDDING_PROVIDER` | `local` | 必须与构建期一致 |
...
4. **域名**:平台会生成公网 HTTPS 地址(如 `<服务名>.zeabur.app` / `<用户名>-ragforge.hf.space`),也可绑定自己的域名。

**平台选择(截至 2026-08)**:Zeabur 的免费共享集群已下线,新项目需租服务器(约 $14/月);免信用卡的免费方案推荐 **Hugging Face Spaces**(Docker CPU 实例,2vCPU/16GB);要求更高可用性与国内延迟可选 Oracle Cloud 免费 VPS(东京/首尔节点)。

**为什么镜像里已带索引?** Dockerfile 构建阶段会重新生成 10 份虚构语料并调用 `build_index.py`,用本地 BGE 建好 FAISS + BM25 索引烤进镜像。索引的 `embedding_sig` 指纹必须与运行时环境变量一致;改了嵌入配置,需在前端「设置」页手动重建索引。

**公网部署须知**:
- 公网必须设置强 `SECRET_KEY`,并确保 HTTPS(平台自带域名默认 HTTPS)。
