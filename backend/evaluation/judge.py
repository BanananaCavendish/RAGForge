...
局限:judge 可能偏宽松/存在位置偏差 → 用金标抽样人工复核。
"""
...
- 回答中的 [1][2] 等序号标注指向参考上下文中的第 1、2 个文件(每个文件可能含多个片段),可用于核对。
...
回答里的 [1]、[2] 等标记指向「参考上下文」中的第 1、2 个文件(每个文件可能含多个片段)。
对回答中出现的每一个引用,判断:该文件(含其全部片段)是否真的支撑了紧邻该引用编号的那句陈述。
只要该文件没有支撑紧邻的陈述(supported=false),即使整段回答没错也一样 false。
